...
impl TransportManager {
    ...
    async fn create_sse_transport(url: impl ToString) -> Result<TransportVariant> {
        ...
        if url_string.contains("0.0.0.0") {
            println!(
                "SSE transport URL contains 0.0.0.0 address. This may cause connection issues if the port is not exposed publicly. Consider using a specific IP address or 'localhost' for local development."
            );
        }
